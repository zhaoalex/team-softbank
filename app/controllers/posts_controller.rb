class PostsController < ApplicationController
  ...
  def index
    ...
    # fresh_when([@posts, @posts.sum(&:like_count), @posts.sum { |p| p.comments_count.to_i }, @posts.map { |p| helpers.distance_of_time_in_words(p.created_at, Time.now) }, @location])
  end
  ...
  def show
    ...
    # fresh_when([@post, @post.like_count, helpers.distance_of_time_in_words(@post.created_at, Time.now), @comments, @comments.sum(&:like_count), @location])
  end
